...
OUT=ROOT/'outputs/ecg-scale-one-window-ops';OUT.mkdir(parents=True,exist_ok=True)
N=22_973_535
DATASETS={
...
for name,path in DATASETS.items():
 ...
 command=[sys.executable,str(RUNNER),'--algorithm=OPS-Miner','--dataset',str(path),
          '--metadata',str(metadata),'--window',str(N),'--step',str(N),
          '--minsup','9189','--output',str(result)]
