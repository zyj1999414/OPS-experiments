...
def read_file(path: str, min_support: float = 4.0):
    global minsup, length, k, fre_num, fre_number, cand_num
...
class OPFOPS:
    """Run OPF independently in every OPS sliding window."""
    ...
    def mine_window(self, window):
        global S, Fmap, allfrepattern, length, k, minsup
        global cand_num, fre_num, fre_number
        global element_num, contrast_num
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="OPF with OPS sliding windows and no reuse"
    )
    parser.add_argument("dataset")
    parser.add_argument("--window", type=int, required=True)
    parser.add_argument("--step", type=int, required=True)
    parser.add_argument("--minsup", "--min-sup", type=float, required=True)
    ...
    print(f"Minimum support: {args.minsup:g}")
