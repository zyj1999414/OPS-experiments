"""OPS-NoReuse: OPS-Miner without length-2 occurrence reuse."""
...
class OPSNoReuseMiner:
    """Rescan length-2 patterns completely in every window."""
    ...
    def find2(self, values=None):
        ...
        position_ids = np.zeros(miner_data.size, dtype=np.int32)
        ids = np.where(
            miner_data[:-1] < miner_data[1:],
            1,
            np.where(miner_data[:-1] > miner_data[1:], 2, 0),
        ).astype(np.int32, copy=False)
        position_ids[1:] = ids
        length_two_counts = np.bincount(
            ids, minlength=3
        ).astype(np.int64, copy=False)
        length_two_counts[0] = 0

        f2 = {}
        if length_two_counts[1] >= self.min_sup:
            f2[(1, 2)] = int(length_two_counts[1])
        if length_two_counts[2] >= self.min_sup:
            f2[(2, 1)] = int(length_two_counts[2])

        if len(f2) != 2:
            active_ids = position_ids.copy()
            keep = np.zeros(3, dtype=bool)
            keep[1] = length_two_counts[1] >= self.min_sup
            keep[2] = length_two_counts[2] >= self.min_sup
    ...
    def _build_next_level(self, pattern_length, current_ids, id_to_pattern):
        ...
        # PPC order: count and prune events before generating any pattern.
        ...
        for group_index in frequent_group_indices.tolist():
            ...
            if candidate is not None:
                # Each frequent (IDp, IDq, C) event uniquely determines
                # one super-pattern, so its event support is the support
                # of that super-pattern.  No second support pruning pass
                # is needed.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="OPS-NoReuse: full length-2 scan per window"
    )
