                            ...
                            if q in suffix_occ_map:
                                ...
                                if prefix_occ_map[p].count(1) >= self.min_sup and suffix_occ_map[q].count(1) >= self.min_sup:
                                    if p[0] != q[-1]:
                                        ...
                                        occ_r, occ_h = self.cal_occ_2(len(p) + 1, prefix_occ_map[p], suffix_occ_map[q])
...
def main():
    ...
    parser.add_argument("dataset", nargs="?", default="/Users/zyj/Downloads/AAPL.txt")
    ...
    # Default to exactly one complete AAPL window.  Command-line overrides
    # remain available for later window experiments.
